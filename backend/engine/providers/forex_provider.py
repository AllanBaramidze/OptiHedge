...
def _sync_forex_quote(symbol: str) -> Optional[Dict[str, Any]]:
    try:
        ...
        # Fix bid/ask fetch to be safer
        info = getattr(ticker, "info", {})
        bid = info.get('bid')
        ask = info.get('ask')

        # PYLANCE FIX: Type-narrowing with isinstance
        ts_name = last.name
...
async def get_forex_historical(symbol: str, period: str = "1d") -> Optional[List[Dict[str, Any]]]:
    """Fetch historical forex data."""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, _sync_forex_historical, symbol, period)

def _sync_forex_historical(symbol: str, period: str) -> Optional[List[Dict[str, Any]]]:
    try:
        yf_symbol = f"{symbol}=X"
        ticker = yf.Ticker(yf_symbol)
